...
obs_diffusive_flux <- h %>% select(mean_diff, sd_diff) %>% na.omit(.) %>%
  ggplot() +
  geom_sf(data = world, lwd = 0.3, color = "black", fill = "white")+
  xlab("Longitude") + ylab("Latitude") +
  labs(title = paste0("    N diffusive observations = 1092"))+
  geom_sf(data = h, pch = 21, color = "black", aes(size = mean_diff, fill = sd_diff))+
...
obs_ebullition_flux <- h %>% select(mean_ebu, sd_ebu) %>% na.omit(.) %>%
  ggplot() +
  geom_sf(data = world, lwd = 0.3, color = "black", fill = "white")+
  xlab("Longitude") + ylab("Latitude") +
  labs(title = paste0("    N diffusive observations = 620"))+
  geom_sf(data = h, pch = 21, color = "black", aes(size = mean_ebu, fill = sd_ebu))+
